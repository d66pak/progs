...
UserInfo::UserInfo(string &line) {
  ...
  // Tokenize the input string and extract the field values
...
ostream& operator<<(ostream& out, const UserInfo& u) {
  out << "|" << u.user << "|" << u.module << "|" << u.partner << "|"
    << u.status << "|" << u.faultStr << "|" << u.timestamp << "|"
    << u.farm << "|" << u.farmHostName << "|" << endl;
}
  ...
  for(int c = 1; c < argc; c++) {
    ...
    switch(*ch) {
      ...
      case 'P':
        while ((ch = argv[++c]) != NULL && *ch != '-') {
          string partner(ch);
          transform (partner.begin(), partner.end(),
              partner.begin(), ::tolower);
          partnerList_.push_back(partner);
        }
        if((ch != NULL) && (*ch == '-')) { c--; }
        break;
...
bool NoteStat::applyFilters(const UserInfo &u) {
  ...
  //cout << "DEBUG: applyFilters " << partnerList_.size() << " " << u.partner << endl;
  // Partner filter
  if (!partnerList_.empty()) {
    string str(u.partner);
    transform (str.begin(), str.end(),
        str.begin(), ::tolower);
    if (partnerList_.end() == find(partnerList_.begin(), partnerList_.end(), str)) {
      return false;
    }
  }
...
void NoteStat::usage() {
  ...
  cout << "notestat -s startTime [options]" << endl;
  cout << "notestat -e endTime -h hrs [options]" << endl;
  ...
  cout << "-p partner" << endl;
  cout << "list of partners separated by space" << endl;
  cout << "like: -p partner_name1 partner_name2" << endl << endl;
    ...
    const boost::shared_ptr<UserInfo>& u2ptr) const
{
  switch (_sortOn) {
    ...
    case NoteStat::kFldPartner:
      return (u1ptr->partner < u2ptr->partner);
