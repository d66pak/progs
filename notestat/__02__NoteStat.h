...
class UserInfo {
  ...
  public:
    ...
    UserInfo(string &line) {
      ...
      char *buf = new char[line.length() + 1];strcpy(buf, line.c_str());
      char *ch1 = strchr(buf, '|');
...
class NoteStat {
  ...
  private:
    enum {
      kFldSTime = 0, kFldUser, kFldModule, kFldStatus, kFldFarm
    };
    ...
    class UserInfoSort {
      public:
        bool operator()(const UserInfo& u1, const UserInfo& u2) const {
          switch (NoteStat::sortField_) {
            case kFldUser:
              return (u1.user < u2.user);
            case kFldModule:
              return (u1.module < u2.module);
            case kFldStatus:
              return (u1.status < u2.status);
            case kFldFarm:
              return (u1.farmHostName < u2.farmHostName);
            default:
              return (u1.ts < u2.ts);
          }
        }
    ...
    typedef boost::shared_ptr<UnserInfoContainer> UserInfoContainerPtr;
  ...
  public:
    ...
    NoteStat(int argc, char **argv) :
      startTime_(0), endTime_(0), count_(false), status_('\0'), tf_(
          UPDATE_FILE), sortField_(kFldSTime) {
        ...
        for (int c = 1; c < argc; c++) {
          ...
          switch (*ch) {
            ...
            case 'G':
              ch = argv[++c];
              setSortField(ch);
              uicp_.reset(new UserInfoContainer());
              break;
    ...
    void print() {
      ...
      if (sortField_ != kFldSTime) {
        copy(uicp_->begin(), uicp_->end(),
            ostream_iterator < UserInfo > (cout, "\n"));
      }
    }
  ...
  private:
    ...
    bool applyFilters(const UserInfo &u) {
      ...
      for (unsigned int i = 0; i < module_.size(); i++) {
        ...
        if (module_[i] == u.module) {
          foundModule = true;
          break;
        }
      }

      if (!foundModule && (module_.size() > 0))
        return false;
    ...
    void usage() {
      ...
      cout
        << "notestat -s startTime [-e endTime] [-c] [-m module] [-o S/R/F] [-u USER_FILE/USER_LIST]"
        << " [-g USER/MODULE/STATUS/FARM]" << endl;
      cout
        << "USER_FILE - name of file having list of user email-ids. Each email-id on separate line"
        << endl;
      cout << "USER_LIST - list of user email-ids separated by single space"
        << endl;
