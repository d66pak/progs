...
sub process_begin($) {
  ...
  unless (MyMailMigrateUtil::lockBtTxRequest(
      $reqName, $lockTime)) {
  ...
  my $lockTimeRef = MyMailMigrateUtil::genLockTimeRef (
    $imapTTL, $reqName,
    $lockTime, $actionTime);
...
sub process_end($) {
  ...
  # Unlock the request.
  MyMailMigrateUtil::unlockBtTxRequest($reqName);
...
sub init($) {
  ...
  MyMailMigrateUtil::setLogHandle($err);
  MyMailMigrateUtil::setVerbose($verbose);
  MyMailMigrateUtil::setLogApplName($applName);
  ...
  # Get specific folders to migrate
  if (exists $mailConf->{specificFolders} && defined $mailConf->{specificFolders}) {

    $specificFolders = $mailConf->{specificFolders};
  }
  ...
  MyFetchEmail::setFolderCharMap($folderCharMap);
  MyFetchEmail::setFolderIgnore($folderIgnore);
  MyFetchEmail::setMsgUniqueIdField("UidValidityUid");
...
sub removeTempMailDir($$$)
{
  ...
  if (($isDSync && $lockMBox) ||
    ...
    MyAppendMessage::recursiveEmptyDir($rmDir);
...
sub fetchMail($$$$$$$$) {
  ...
  if ($childPid) {
    ...
    MyFetchEmail::FetchMailIMAP($logger, $user, $sid, $mssilo, $id, '', $imapServer, $imapPort, $dir,
...
sub writeMail($$$$$$) {
  ...
  MyAppendMessage::AppendMessage($logger, $user, $sid, $mssilo, $dir,
...
sub getGmailFolderList
{
  my ($imap) = @_;

  print "Getting list of gmail folders...\n";
  my $folderAndSeps = MyFetchEmail::getAllFolders($imap);
  ...
  foreach my $fs (@$folderAndSeps) {
    ...
    if (MyFetchEmail::isFolderIgnored($yFolder)) {

      # Skip ignored folders
      next;
    }

    push (@gmailFolderList, $yFolder);
    #print $FH "\'$yFolder\',\n";
  }
  return \@gmailFolderList;
}

sub runFolderComparison
{
  my ($user) = @_;

  my $cmd = "./gmail-y-summary.pl -u $user";
  my $ret = qx/$cmd/;
  unless ($?) {

    print "Folder comparison success...\n";
  }
}
...
my ($sid, $silo) = findSidSilo($currUser);
...
unless (defined $userFolders && scalar (@$userFolders)) {

# If nothing specified in conf file fetch from gmail
  my $imap = MyFetchEmail::openIMAPConnection($currUser, '', $imapServer, $imapPort, $imapSSL, $imapPeek, 'XOAUTH', $xoauth_b64_encoded);

  if (defined $imap) {

    $userFolders = getGmailFolderList($imap);
    MyFetchEmail::closeIMAPConnection($imap);
  }
  else {
...
my @folderListToMigrate;

if (defined $specificFolders && scalar (@$specificFolders)) {

  @folderListToMigrate = @$specificFolders;
}
else {

  @folderListToMigrate = @$userFolders;
}

print "Folders to migrate: @folderListToMigrate\n";
...
while ($keepMigrating) {
  ...
  my $folderToMigrate = pop (@folderListToMigrate);
  my @folderListToIgnore = @$folderIgnore;

  if (defined $folderToMigrate) {

    #print "Migrating folder: $folderToMigrate\n";
    foreach my $f (@$userFolders) {

      if ($f ne $folderToMigrate) {

        push (@folderListToIgnore,  $f);
      }
    }
  }
  else {

    last if (defined $specificFolders && scalar (@$specificFolders));

    # All folders are migrated
    # Final migration with original ignore list
    print "Final migration\n";
    $folderToMigrate = 'All Folders';
    $keepMigrating = 0;
  }

  print "Ignored folders: @folderListToIgnore\n";
  MyFetchEmail::setFolderIgnore(\@folderListToIgnore);

  # Start migratining....
  my ($retVal, $mboxSize, $errormsg, $attempt) = (1, 0, '', 1);
  while ($retVal) {

    print "Folder: $folderToMigrate Attempt $attempt:  Fetching mailbox for SID $sid SILO $silo\n";
    ($retVal, $mboxSize, $errormsg) = fetchMail('Sky', $currUser, $sid, $silo, $currUser, $consumer_secret, 0, $consumer_key);

    print "User: $currUser Status: $retVal MBox size: $mboxSize Error: $errormsg\n";
    ...
    if (defined $sendInternalNotif && $sendInternalNotif eq 'yes') {
      ...
      my $url = "http://mrs01.mail.sp1.yahoo.com/lca/report?partner=Sky&app=SkyMailResync&rc=$rc&uid=$currUser&seplogfile=true&fs=" . uri_escape($notifMsg);
