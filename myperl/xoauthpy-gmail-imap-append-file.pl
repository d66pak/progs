...
my $emailId = 'gmailuser22@sso-test-test.sky.com';
...
$client->select('INBOX') or die "Cannot select INBOX folder: $@\n";

=begin comment
print "Start appending msg to INBOX...\n";
for (my $count = 0; $count < 1; $count++) {

  my $uid = $client->append_file('INBOX', 'inbox0.msg')
    or die "Could not append_file: $@\n";
  print "Appended $count msg uid: " . $uid . "\n";
  print "Messages in INBOX " . $client->message_count() . "\n";
  #sleep(1);
}
=cut
